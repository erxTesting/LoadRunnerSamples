...
SET version=0.004-beta  &rem 20250505 erx    Updated static information, added comments, and improved functionalit
...
set		loopyEditor_choice=,notepad,
...
start "edit list" %loopyEditor% "%loopyList%."
:loop_1
:loop_2
:loop_3
goto:eof
...
::cls
echo Enter the file name to open for all scripts & echo Examples- output.txt, vuser_init.c, vuser_end.c, Action.c, pre_cci.c, etc.
set /p fileOpen=(current "%fileOpen%"): 
goto:eof
:loop_O
echo %~1\%fileOpen%
start "open file(s) %fileOpen%" /min cmd /c "explorer %~1\%fileOpen%"
goto:eof
